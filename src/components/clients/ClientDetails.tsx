...
const ClientDetails: React.FC<Props> = ({ client }) => (
  <div className="p-4 bg-white rounded shadow">
    ...
    <div className="text-gray-700">Dirección: {client.address ? JSON.stringify(client.address) : 'No especificada'}</div>
    <div className="text-gray-700">Estado: {typeof client.status === 'string' ? client.status : client.status?.name || 'No especificado'}</div>
